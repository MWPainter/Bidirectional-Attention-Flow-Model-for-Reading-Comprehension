...
tf.app.flags.DEFINE_integer("batch_size", 29, "Batch size to use during training.")
...
tf.app.flags.DEFINE_string("model_version", "_2", "Make this '' for initial model, if we ever want to retrain a model, then we can use this (with '_i') to not overwrite the original data")
...
tf.app.flags.DEFINE_integer("epoch_base", 3, "The first epoch, so that we are saving the correct model and outputting the correct numbers if we restarted")
